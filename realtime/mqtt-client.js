...
import { handleMqttMessage, extractDeviceIdFromTopic } from './mqtt-handlers.js';
import { enqueueDeviceIngest } from '../lib/device-ingest-queue.js';
...
export function initMqttClient() {
  ...
  // --- Message (serial per device — elak race dedup) ---
  client.on('message', (topic, payloadBuffer) => {
    const deviceId = extractDeviceIdFromTopic(topic);
    enqueueDeviceIngest(deviceId, () =>
      handleMqttMessage(topic, payloadBuffer, client)
    ).catch((err) => {
