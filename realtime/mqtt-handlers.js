...
export async function handleMqttMessage(topic, payloadBuffer, client) {
  ...
  switch (kind) {
    ...
    // ========================================
    case 'status': {
      await processTracking(payload, DATA_SOURCE.MQTT, { sourceKind: 'status' });
      break;
    }
    ...
    case 'backfill': {
      ...
      console.log(
        `[mqtt] Backfill ${deviceId}: ${result.deduped}/${result.total} disimpan (${result.skipped} redundan)`
      );
    ...
    // ========================================
    case 'gateway': {
      await processTracking(payload, DATA_SOURCE.MQTT, { sourceKind: 'gateway' });
      break;
    }
