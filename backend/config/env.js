...
export const env = {
  ...
  // MQTT (grouped for readability)
  MQTT: {
    ENABLED: bool('MQTT_ENABLED', false),
    HOST: need('MQTT_HOST', '178.128.48.114'),
    PORT: int('MQTT_PORT', 8885),
    PROTOCOL: need('MQTT_PROTOCOL', 'ws'),
    USERNAME: need('MQTT_USERNAME', ''),
    PASSWORD: need('MQTT_PASSWORD', ''),

    WSS_HOST: need('MQTT_WSS_HOST', '178.128.48.114'),
    WSS_PORT: int('MQTT_WSS_PORT', 8887),

    PROCESS_CONCURRENCY: int('MQTT_PROCESS_CONCURRENCY', 8),
    SUBSCRIBE_QOS: int('MQTT_SUBSCRIBE_QOS', 2),
    MODBUS_SUBSCRIBE_QOS: int('MQTT_MODBUS_SUBSCRIBE_QOS', 1),
    DEVICE_UPDATE_MIN_MS: int('MQTT_DEVICE_UPDATE_MIN_MS', 0),
